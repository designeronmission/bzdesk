...
(function ($) {
	...
	$(document).ready(function ($) {
		...
		$(window).scroll(function () {
			...
			if (scroll > 300) {
				$(".bz-header-area.header-sticky").addClass("sticky");
				$(".bz-header-area.header-sticky").removeClass("sticky-out");
			} else if (scroll < lastScrollTop) {
				if (scroll < 500) {
					$(".bz-header-area.header-sticky").addClass("sticky-out");
					$(".bz-header-area.header-sticky").removeClass("sticky");
				}
			} else {
				$(".bz-header-area.header-sticky").removeClass("sticky");
			}
		...
		// Portfolio filter js
		$(".bz-project-4-wrappper").isotope({
			percentPosition: true,
			masonry: {
				// use outer width of grid-sizer for columnWidth
				columnWidth: ".bz-project-4-item",
				gutter: 125,
			},
		});
		$(".bz-project-4-wrappper").imagesLoaded(function () {
			var $grid = $(".bz-project-4-wrappper").isotope({
				itemSelector: ".bz-project-4-item",
				percentPosition: true,
				masonry: {
					columnWidth: ".bz-project-4-item",
					gutter: 125,
		...
		// Post Carousel Js
		$(".bz-post__gallery.owl-carousel").owlCarousel({
			items: 1,
			loop: true,
			margin: 30,
			dots: false,
		...
		// Project Slider Js
		var slider = new Swiper(".bz-project-9-active", {
			slidesPerView: 3,
			spaceBetween: 30,
			loop: true,
			speed: 2000,
			...
			centeredSlides: true,
			navigation: {
				prevEl: ".bz-project-9-prev",
				nextEl: ".bz-project-9-next",
			},
			pagination: {
				el: ".bz-project-9-pagination",
				clickable: true,
			},
			breakpoints: {
				1200: {
		...
		var testimonialSwiper = new Swiper(".bz-testimonial-7-active", {
			slidesPerView: 1,
			spaceBetween: 30,
			loop: true,
			centeredSlides: true,
		...
		var slider = new Swiper(".bz-testimonial-9-active", {
			direction: "vertical",
			slidesPerView: 1.8,
			spaceBetween: 30,
			centeredSlides: true,
		...
		var testimonial = new Swiper(".bz-testimonial-10-active", {
			slidesPerView: 1,
			spaceBetween: 30,
			loop: true,
			centeredSlides: true,
		...
		// Testimonial Slider Js
		if ($(".bz-testimonial-slider").length > 0) {
			var brand = new Swiper(".bz-testimonial-slider", {
				slidesPerView: 3,
				spaceBetween: 30,
				loop: true,
				autoplay: {
		...
		// Testimonial Slider Js
		if ($(".bz-testimonial-slider8").length > 0) {
			var brand = new Swiper(".bz-testimonial-slider8", {
				slidesPerView: 3,
				spaceBetween: 30,
				active: true,
				loop: true,
	...
	$(window).on("load", function () {
		...
		$(".accordion-list > li:nth-child(1)")
			.addClass("active")
			.find(".bz-service-5-accordion-wrap")
			.show();
		$(
			".accordion-list > li:not(:nth-child(1)) .bz-service-5-accordion-wrap"
		).hide();
		$(".accordion-list > li").click(function () {
			if ($(this).hasClass("active")) {
				...
				$(".accordion-list > li.active .bz-service-5-accordion-wrap").slideUp();
...
********************************************************************/
	...
	*/
	function heroAreaAnimation() {
		...
		let hero4SubTitle = $(
			".activeAnimation .bz-hero-4-subtitle, .activeAnimation .bz-hero-7-thumb"
		);
		let hero4Title = $(
			".activeAnimation .bz-hero-4-title, .activeAnimation .bz-hero-7-content"
		);
		let hero4Thumb = $(
			".activeAnimation .bz-hero-4-bottom-thumb img, .activeAnimation .bz-hero-7-button"
		);
		let hero4Shape1 = $(
			".activeAnimation .bz-hero-4-bottom-thumb-shape-1, .activeAnimation .bz-feature-7-thumb"
		);
		let hero4Shape2 = $(
			".activeAnimation .bz-hero-4-bottom-thumb-shape-2, .activeAnimation .bz-feature-7-wrapper"
		);
		let hero4Reviews = $(".activeAnimation .bz-hero-4-bottom-reviews");
		let hero4Counter = $(".activeAnimation .bz-hero-4-bottom-counter");
	...
	$('.bz-btn-rounded').on('mouseenter', function (e) {
		var x = e.pageX - $(this).offset().left;
		var y = e.pageY - $(this).offset().top;
		$(this).find('.bz-btn-circle-dot').css({
			top: y,
			left: x
		});
	});
	$('.bz-btn-rounded').on('mouseout', function (e) {
		var x = e.pageX - $(this).offset().left;
		var y = e.pageY - $(this).offset().top;
		$(this).find('.bz-btn-circle-dot').css({
			top: y,
	...
	*/
	...
	// splitText
	if ($(".bz-char-animation").length > 0) {
		let char_come = gsap.utils.toArray(".bz-char-animation");
	...
	// Text Invert
	const split = new SplitText(".bz-text-invert", { type: "lines" });
	...
	// line 3d
	let bz_title_anim = gsap.utils.toArray(".bz_title_anim");
	bz_title_anim.forEach(splitTextLine => {
		var delay_value = 0.5;
...
let formats = document.querySelectorAll(".section");

let horizontalScroll = gsap.to(formats, {
...
ScrollTrigger.create({
  ...
  trigger: "#middle",
  start: "left center",
  onEnter: self => text.innerText = "Middle",
  onLeaveBack: self => text.innerText = "Start"
});
...
ScrollTrigger.create({
  ...
  trigger: "#end",
  start: "left center",
  onEnter: self => text.innerText = "End",
  onLeaveBack: self => text.innerText = "Middle"
});
...
var buttons = gsap.utils.toArray('.active-btn');
...
function stopOverscroll(element) {
  ...
  if (/^((?!chrome|android).)*safari/i.test(navigator.userAgent)) {
    ...
    element.addEventListener("wheel", kill, {passive: false});
    element.addEventListener("touchmove", kill, {passive: false});
  }
}
